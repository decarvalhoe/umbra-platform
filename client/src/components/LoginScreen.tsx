...
export function LoginScreen({ onLogin }: LoginScreenProps) {
  ...
  return (
    <div className="ls-root">
      ...
      <div className="ls-rune-bg" style={{ transform: `translate(-50%, -50%) rotate(${runeRot}deg)` }} />
      <div className="ls-rune-bg ls-rune-bg--inner" style={{ transform: `translate(-50%, -50%) rotate(${-runeRot * 1.4}deg)` }} />
      ...
      <div className={`ls-panel ${visible ? "ls-panel--visible" : ""}`}>
        ...
        <div className="ls-tabs">
          <button className={`ls-tab ${mode === "login" ? "ls-tab--active" : ""}`}
            onClick={() => { setMode("login"); setError(null); }}>
            Connexion
          </button>
          <button className={`ls-tab ${mode === "register" ? "ls-tab--active" : ""}`}
        ...
        <form onSubmit={handleSubmit} className="ls-form">
          <div className="ls-field">
            ...
            <div className="ls-input-wrap">
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                placeholder="chasseur@umbra.gg" required className="ls-input" autoComplete="email" />
            </div>
          </div>
          <div className="ls-field">
            ...
            <div className="ls-input-wrap">
              ...
              <input type={showPwd ? "text" : "password"} value={password}
                ...
                required className="ls-input"
                autoComplete={mode === "login" ? "current-password" : "new-password"} />
              <button type="button" className="ls-pwd-toggle"
          ...
          {error && (
            <div className="ls-error">
              <span className="ls-error-icon">⚠</span>{error}
            </div>
          )}
        ...
        <button className="ls-guest" onClick={async () => {
          setLoading(true);
          try {
            await authenticateEmail(`guest_${Date.now()}@umbra.gg`, "guest_pass_umbra_2026", true);
            onLogin();
          } catch { setError("Connexion invité indisponible"); }
