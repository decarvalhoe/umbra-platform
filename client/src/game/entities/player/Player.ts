...
export class Player extends Phaser.Physics.Arcade.Sprite {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number) {
    ...
    const texKey = scene.textures.exists('player_sheet') ? 'player_sheet' : 'player'
    if (!scene.textures.exists('player_sheet') && !scene.textures.exists('player')) {
    ...
    // Only tint fallback rectangle
    if (texKey === 'player') this.setTint(0x4488ff)
  ...
  /** Play a named animation if the sprite sheet is loaded. */
